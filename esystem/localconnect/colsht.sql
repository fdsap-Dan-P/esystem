...
acc AS (
  SELECT
    acc.id Account_Id, cus.iiid, cus.id customerid, cus.central_office_id centralofficeid, 
    cus.cid, acc.acc, acc.alternate_acc alternateacc,
...
--            a.balance,
  ...
  --  CASE WHEN a.apptype = 3 THEN acc.principal - acc.prin ELSE 0::numeric END AS lbalprin,
   --         COALESCE(grp_1.code, 0.5) AS grpcode,
    ...
    csn.value SharesOfStock, acc.Uuid
  FROM Customer cus
...
SELECT 
  ...
  SharesOfStock, datestablished DateEstablished, Classification, ClassDesc, WriteOff, Uuid
FROM sht;
