...
INSERT INTO MapBranch(IIID, BrCode, NewBrCode)
 ...
 UPDATE staging.stg_brcode sb set officeiiid=467850 where brcode = 'C1';
 INSERT INTO MapBranch(IIID,BrCode, NewBrCode)
 -- Copy to MapBranch
...
INSERT INTO MapCenter(IIID, BrCode, UnitIIID, CenterCode)
  ...
  from staging.stg_unit sc
  inner join staging.stg_center cen on cen.BRCode = sc.brCode and cen.unitcode = sc.unitcode 
  inner join staging.stg_customer cus on cen.brcode = cus.brcode and cen.centercode = cus.center_code 
  ...
  from staging.stg_center sc
  inner join staging.stg_customer cus on sc.brcode = cus.brcode and sc.centercode = cus.center_code 
  ...
  select brcode from staging.stg_unit group by brcode
    select brcode from staging.stg_center group by brcode
    
     select distinct cus.center_code 
     from staging.stg_customer cus
     left join staging.stg_center cen on cen.brcode = cus.brcode and cen.centercode = cen.centercode 
     ...
     */
     select brcode from staging.stg_customer group by brcode
  /*
    
  update staging.stg_center set unitcode = 524
    from staging.stg_unit sc
  inner join staging.stg_center cen on cen.BRCode = sc.brCode and cen.unitcode = sc.unitcode 
  inner join staging.stg_customer cus on cen.brcode = cus.brcode and cen.centercode = cus.center_code 
  LEFT join mapunit m on m.brcode = sc.brcode and sc.unitcode = m.unitcode 
  LEFT JOIN Offices o on o.officeiiid = m.iiid
  where staging.stg_center.brcode = sc.brcode and staging.stg_center.unitcode = sc.unitcode and sc.unit = 'RESERVED'
  */
...
--// get Units from CORE
  ...
  LEFT JOIN staging.stg_unit sc on sc.brcode = mu.brcode and sc.unitcode = mu.unitcode 
  ...
  select * from staging.stg_unit where brcode = '05' and unitcode = 535
...
FROM
 (SELECT 
  ...
  --WHERE iiid.iiid in (2514424,3432593)
...
FROM staging.stg_saMaster WHERE BrCode in ('04','05','07')
ON CONFLICT (BrCode, Acc) 
 DO NOTHING;
...
inner join staging.stg_samaster ss on ss.brcode = iiid.brcode and ss.cid = iiid.cid
left join accounts a on a.cid = iiid.iiid
where 
iiid.iiid in (2514424,3432593)
...
select a.AcctType from staging.stg_lnMaster a
left join  AcctParms y on y.AcctType = a.AcctType
where y.accttype is null
group by a.AcctType 
--select brcode from staging.stg_lnMaster group by brcode
...
select * from staging.stg_customer where brcode = '05' and cid = 1981257
...
--and iiid.iiid in (2514424,3432593)
...
INSERT INTO ArchivedLoanInst(dNum, Acc, DueDate, Prin, IntR, UpInt)
  SELECT i.dNum, m.NewAcc, DueDate, i.Prin, i.IntR, i.UpInt
  ...
  INNER JOIN CIDs on CIDs.cid = a.CID
...
INSERT INTO LoanInst( dnum, acc, duedate, instflag, prin, intr, oth, penalty, endbal, endint, endoth, instpd, penpd, carval, upint, servfee, pledgeamort)
  ...
  INNER JOIN Accounts a on a.acc = m.newacc
  INNER JOIN CIDs on CIDs.cid = a.CID
  WHERE m.Archived = FALSE --and i.BrCode in ('04','05','07') 
  -- and a.cid in (2514424,3432593)
  ON CONFLICT (Acc, dNum) DO NOTHING; 
...
FROM
  ...
  INNER JOIN CIDs on CIDs.cid = acc.CID
  WHERE a.Archived = FALSE --and a.BrCode in ('04','05','07') 
  --and acc.cid in (2514424,3432593)
...
inner join staging.stg_loaninst sl on sl.brcode = ma.brcode and sl.acc  = ma.acc
