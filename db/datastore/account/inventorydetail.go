...
func (q *QueriesAccount) CreateInventoryDetail(ctx context.Context, arg InventoryDetailRequest) (model.InventoryDetail, error) {
...
func (q *QueriesAccount) DeleteInventoryDetail(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, deleteInventoryDetail, id)
	return err
}
...
const getInventoryDetailSQL = `SELECT 
...
func populateInventoryDetails(q *QueriesAccount, ctx context.Context, sql string, param ...interface{}) ([]InventoryDetailInfo, error) {
...
func (q *QueriesAccount) GetInventoryDetail(ctx context.Context, id int64) (InventoryDetailInfo, error) {
	script := fmt.Sprintf(`%v WHERE d.id = $1`, getInventoryDetailSQL)
	...
	if len(items) > 0 {
		...
		return InventoryDetailInfo{}, fmt.Errorf("inventory Detail ID:%v not found", id)
	}
}

func (q *QueriesAccount) GetInventoryDetailbyUuid(ctx context.Context, uuid uuid.UUID) (InventoryDetailInfo, error) {
	script := fmt.Sprintf(`%v WHERE mr.uuid = $1`, getInventoryDetailSQL)
	log.Printf("sql: %v", script)
	...
	if len(items) > 0 {
		...
		return InventoryDetailInfo{}, fmt.Errorf("inventory Detail UUID:%v not found", uuid)
	}
}

func (q *QueriesAccount) GetInventoryDetailbyName(ctx context.Context, name string) (InventoryDetailInfo, error) {
	script := fmt.Sprintf(`%v WHERE Title = $1 LIMIT 1`, getInventoryDetailSQL)
	log.Printf("sql: %v", script)
	items, err := populateInventoryDetails(q, ctx, script, name)
	log.Printf("items: %v", items)
	if len(items) > 0 {
		return items[0], err
	} else {
		return InventoryDetailInfo{}, fmt.Errorf("inventory Detail Name:%v not found", name)
	}
}
...
func (q *QueriesAccount) ListInventoryDetail(ctx context.Context, arg ListInventoryDetailParams) ([]InventoryDetailInfo, error) {
	sql := fmt.Sprintf(`%v WHERE Account_Inventory_Id = $1
	ORDER BY id LIMIT $2 OFFSET $3 `, getInventoryDetailSQL)
	return populateInventoryDetails(q, ctx, sql, arg.AccountInventoryId, arg.Limit, arg.Offset)
}
...
func (q *QueriesAccount) UpdateInventoryDetail(ctx context.Context, arg InventoryDetailRequest) (model.InventoryDetail, error) {
...
func (store *SQLStoreAccount) CreateInventoryDetailFull(ctx context.Context, arg InventoryDetailFullRequest) (model.InventoryDetail, error) {
