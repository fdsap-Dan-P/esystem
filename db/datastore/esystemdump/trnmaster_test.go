...
func TestTrnMaster(t *testing.T) {
	...
	// Get Data
	getData1, err1 := testQueriesDump.GetTrnMaster(context.Background(), d1.BrCode, d1.TrnDate, d1.Trn)
	...
	getData2, err2 := testQueriesDump.GetTrnMaster(context.Background(), d2.BrCode, d2.TrnDate, d2.Trn)
	...
	getData1, err1 = testQueriesDump.GetTrnMaster(context.Background(), updateD2.BrCode, updateD2.TrnDate, updateD2.Trn)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteTrnMaster(t, d1.BrCode, d1.TrnDate, d1.Trn)
	testDeleteTrnMaster(t, d2.BrCode, d2.TrnDate, d2.Trn)
}

func testListTrnMaster(t *testing.T, arg ListTrnMasterParams) {

	TrnMaster, err := testQueriesDump.ListTrnMaster(context.Background(), int64(0))
	require.NoError(t, err)
	// fmt.Printf("%+v\n", TrnMaster)
...
func randomTrnMaster() model.TrnMaster {
...
func createTestTrnMaster(
	t *testing.T,
	req model.TrnMaster) error {

	err1 := testQueriesDump.CreateTrnMaster(context.Background(), req)
	...
	getData, err2 := testQueriesDump.GetTrnMaster(context.Background(), req.BrCode, req.TrnDate, req.Trn)
	require.NoError(t, err2)
...
func updateTestTrnMaster(
	t *testing.T,
	d1 model.TrnMaster) error {

	err := testQueriesDump.UpdateTrnMaster(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteTrnMaster(t *testing.T, brCode string, trnDate time.Time, trn int64) {
	err := testQueriesDump.DeleteTrnMaster(context.Background(), brCode, trnDate, trn)
	require.NoError(t, err)

	ref1, err := testQueriesDump.GetTrnMaster(context.Background(), brCode, trnDate, trn)
	require.Error(t, err)
