...
func TestUnit(t *testing.T) {
	...
	// Get Data
	getData1, err1 := testQueriesDump.GetUnit(context.Background(), d1.BrCode, d1.UnitCode)
	...
	getData2, err2 := testQueriesDump.GetUnit(context.Background(), d2.BrCode, d2.UnitCode)
	...
	getData1, err1 = testQueriesDump.GetUnit(context.Background(), updateD2.BrCode, updateD2.UnitCode)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteUnit(t, d1.BrCode, d1.UnitCode)
	testDeleteUnit(t, d2.BrCode, d2.UnitCode)
}

func testListUnit(t *testing.T, arg ListUnitParams) {

	Unit, err := testQueriesDump.ListUnit(context.Background(), int64(0))
	require.NoError(t, err)
	// fmt.Printf("%+v\n", Unit)
...
func createTestUnit(
	t *testing.T,
	req model.Unit) error {

	err1 := testQueriesDump.CreateUnit(context.Background(), req)
	...
	getData, err2 := testQueriesDump.GetUnit(context.Background(), req.BrCode, req.UnitCode)
	require.NoError(t, err2)
...
func updateTestUnit(
	...
	err := testQueriesDump.UpdateUnit(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteUnit(t *testing.T, brCode string, UnitCode int64) {
	err := testQueriesDump.DeleteUnit(context.Background(), brCode, UnitCode)
	require.NoError(t, err)

	ref1, err := testQueriesDump.GetUnit(context.Background(), brCode, UnitCode)
	require.Error(t, err)
