...
func TestCenterWorker(t *testing.T) {
	...
	// Get Data
	getData1, err1 := testQueriesDump.GetCenterWorker(context.Background(), d1.BrCode, d1.AOID)
	...
	getData2, err2 := testQueriesDump.GetCenterWorker(context.Background(), d2.BrCode, d2.AOID)
	...
	getData1, err1 = testQueriesDump.GetCenterWorker(context.Background(), updateD2.BrCode, updateD2.AOID)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteCenterWorker(t, d1.BrCode, d1.AOID)
	testDeleteCenterWorker(t, d1.BrCode, d2.AOID)
}

func testListCenterWorker(t *testing.T, arg ListCenterWorkerParams) {

	CenterWorker, err := testQueriesDump.ListCenterWorker(context.Background(), int64(0))
	require.NoError(t, err)
	// fmt.Printf("%+v\n", CenterWorker)
...
func randomCenterWorker() model.CenterWorker {

	arg := model.CenterWorker{
		ModCtr:      1,
		BrCode:      "01",
		AOID:        69,
...
func createTestCenterWorker(
	t *testing.T,
	req model.CenterWorker) error {

	err1 := testQueriesDump.CreateCenterWorker(context.Background(), req)
	...
	getData, err2 := testQueriesDump.GetCenterWorker(context.Background(), req.BrCode, req.AOID)
	require.NoError(t, err2)
...
func updateTestCenterWorker(
	t *testing.T,
	d1 model.CenterWorker) error {

	err := testQueriesDump.UpdateCenterWorker(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteCenterWorker(t *testing.T, brCode string, AOID int64) {
	err := testQueriesDump.DeleteCenterWorker(context.Background(), brCode, AOID)
	require.NoError(t, err)

	ref1, err := testQueriesDump.GetCenterWorker(context.Background(), brCode, AOID)
	require.Error(t, err)
