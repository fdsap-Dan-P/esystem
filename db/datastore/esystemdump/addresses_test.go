...
func TestAddresses(t *testing.T) {
	...
	err := createTestAddresses(t, d1)
	require.NoError(t, err)

	err = createTestAddresses(t, d2)
	require.NoError(t, err)

	// Get Data
	getData1, err1 := testQueriesDump.GetAddresses(context.Background(), d1.BrCode, d1.SeqNum)
	...
	getData2, err2 := testQueriesDump.GetAddresses(context.Background(), d2.BrCode, d2.SeqNum)
	...
	getData1, err1 = testQueriesDump.GetAddresses(context.Background(), updateD2.BrCode, updateD2.SeqNum)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteAddresses(t, d1.BrCode, d1.SeqNum)
	testDeleteAddresses(t, d2.BrCode, d2.SeqNum)
}

func testListAddresses(t *testing.T, arg ListAddressesParams) {

	Addresses, err := testQueriesDump.ListAddresses(context.Background(), int64(0))
	require.NoError(t, err)
	// fmt.Printf("%+v\n", Addresses)
...
func randomAddresses() model.Addresses {
...
func createTestAddresses(
	t *testing.T,
	req model.Addresses) error {

	err1 := testQueriesDump.CreateAddresses(context.Background(), req)
	// fmt.Printf("Get by createTestAddresses%+v\n", getData1)
	require.NoError(t, err1)

	getData, err2 := testQueriesDump.GetAddresses(context.Background(), req.BrCode, req.SeqNum)
	require.NoError(t, err2)
...
func updateTestAddresses(
	t *testing.T,
	d1 model.Addresses) error {

	err := testQueriesDump.UpdateAddresses(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteAddresses(t *testing.T, brCode string, SeqNum int64) {
	err := testQueriesDump.DeleteAddresses(context.Background(), brCode, SeqNum)
	require.NoError(t, err)

	ref1, err := testQueriesDump.GetAddresses(context.Background(), brCode, SeqNum)
	require.Error(t, err)
