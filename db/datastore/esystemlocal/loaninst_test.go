...
func TestLoanInst(t *testing.T) {
	...
	// Get Data
	getData1, err1 := testQueriesLocal.GetLoanInst(context.Background(), d1.Acc, d1.Dnum)
	...
	getData2, err2 := testQueriesLocal.GetLoanInst(context.Background(), d2.Acc, d2.Dnum)
	...
	getData1, err1 = testQueriesLocal.GetLoanInst(context.Background(), updateD2.Acc, updateD2.Dnum)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteLoanInst(t, d1.Acc, d1.Dnum)
	testDeleteLoanInst(t, d2.Acc, d2.Dnum)
}

func testListLoanInst(t *testing.T, arg ListLoanInstParams) {

	LoanInst, err := testQueriesLocal.ListLoanInst(context.Background())
	require.NoError(t, err)
	// fmt.Printf("%+v\n", LoanInst)
...
func randomLoanInst() LoanInstRequest {
...
func createTestLoanInst(
	t *testing.T,
	req LoanInstRequest) error {

	err1 := testQueriesLocal.CreateLoanInst(context.Background(), req)
	...
	getData, err2 := testQueriesLocal.GetLoanInst(context.Background(), req.Acc, req.Dnum)
	require.NoError(t, err2)
...
func updateTestLoanInst(
	t *testing.T,
	d1 LoanInstRequest) error {

	err := testQueriesLocal.UpdateLoanInst(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteLoanInst(t *testing.T, Acc string, dNum int64) {
	err := testQueriesLocal.DeleteLoanInst(context.Background(), Acc, dNum)
	require.NoError(t, err)

	ref1, err := testQueriesLocal.GetLoanInst(context.Background(), Acc, dNum)
	require.Error(t, err)
