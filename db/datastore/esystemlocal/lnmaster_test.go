...
func TestCSVLnMaster(t *testing.T) {
	err := testQueriesLocal.LnMasterCSV(context.Background(), "csv/LnMaster.csv")
	if err != nil {
		log.Fatal(err)
	}

	cmd := exec.Command("/bin/sh", "-c", "docker cp /Users/rhickmercado/Documents/Programming/go/src/simplebank/db/datastore/esystemlocal/csv/LnMaster.csv 263d578de57f:/var/lib/postgresql/LnMaster.csv")
	err = cmd.Run()
...
func TestLnMaster(t *testing.T) {
	...
	// Get Data
	getData1, err1 := testQueriesLocal.GetLnMaster(context.Background(), d1.Acc)
	...
	getData2, err2 := testQueriesLocal.GetLnMaster(context.Background(), d2.Acc)
	...
	getData1, err1 = testQueriesLocal.GetLnMaster(context.Background(), updateD2.Acc)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteLnMaster(t, d1.Acc)
	testDeleteLnMaster(t, d2.Acc)
}

func testListLnMaster(t *testing.T, arg ListLnMasterParams) {

	LnMaster, err := testQueriesLocal.ListLnMaster(context.Background())
	require.NoError(t, err)
	// fmt.Printf("%+v\n", LnMaster)
...
func randomLnMaster() LnMasterRequest {
...
func createTestLnMaster(
	t *testing.T,
	req LnMasterRequest) error {

	err1 := testQueriesLocal.CreateLnMaster(context.Background(), req)
	...
	getData, err2 := testQueriesLocal.GetLnMaster(context.Background(), req.Acc)
	require.NoError(t, err2)
...
func updateTestLnMaster(
	t *testing.T,
	d1 LnMasterRequest) error {

	err := testQueriesLocal.UpdateLnMaster(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteLnMaster(t *testing.T, Acc string) {
	err := testQueriesLocal.DeleteLnMaster(context.Background(), Acc)
	require.NoError(t, err)

	ref1, err := testQueriesLocal.GetLnMaster(context.Background(), Acc)
	require.Error(t, err)
