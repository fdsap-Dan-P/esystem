...
func TestSaTrnMaster(t *testing.T) {
	...
	trn, _ := testQueriesLocal.CodeIncrement(context.Background(), "saTrnMaster", `1999-01-02`)
	d2.Trn = trn
	...
	// Get Data
	getData1, err1 := testQueriesLocal.GetSaTrnMaster(context.Background(), d1.TrnDate, d1.Trn)
	...
	getData2, err2 := testQueriesLocal.GetSaTrnMaster(context.Background(), d2.TrnDate, d2.Trn)
	...
	getData1, err1 = testQueriesLocal.GetSaTrnMaster(context.Background(), updateD2.TrnDate, updateD2.Trn)
	require.NoError(t, err1)
	...
	// Delete Data
	testDeleteSaTrnMaster(t, d1.TrnDate, d1.Trn)
	testDeleteSaTrnMaster(t, d1.TrnDate, d2.Trn)
}

func testListSaTrnMaster(t *testing.T, arg ListSaTrnMasterParams) {

	SaTrnMaster, err := testQueriesLocal.ListSaTrnMaster(context.Background())
	require.NoError(t, err)
	// fmt.Printf("%+v\n", SaTrnMaster)
...
func randomSaTrnMaster() SaTrnMasterRequest {

	trn, _ := testQueriesLocal.CodeIncrement(context.Background(), "saTrnMaster", `1999-01-02`)

	arg := SaTrnMasterRequest{
...
func createTestSaTrnMaster(
	t *testing.T,
	req SaTrnMasterRequest) error {

	err1 := testQueriesLocal.CreateSaTrnMaster(context.Background(), req)
	...
	getData, err2 := testQueriesLocal.GetSaTrnMaster(context.Background(), req.TrnDate, req.Trn)
	require.NoError(t, err2)
...
func updateTestSaTrnMaster(
	t *testing.T,
	d1 SaTrnMasterRequest) error {

	err := testQueriesLocal.UpdateSaTrnMaster(context.Background(), d1)
	require.NoError(t, err)

	return err
}

func testDeleteSaTrnMaster(t *testing.T, trnDate time.Time, trn int64) {
	err := testQueriesLocal.DeleteSaTrnMaster(context.Background(), trnDate, trn)
	require.NoError(t, err)

	ref1, err := testQueriesLocal.GetSaTrnMaster(context.Background(), trnDate, trn)
	require.Error(t, err)
